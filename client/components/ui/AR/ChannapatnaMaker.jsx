...
// // //         </Canvas>
// // //       </div>
// // //       <div className="h-[40%] w-full bg-stone-950 border-t border-yellow-600/30 p-4 flex flex-col items-center justify-center text-center">
...
// // //              <button onClick={() => step < 3 && setStep(step + 1)} disabled={step === 3} className={`flex-[2] py-3 rounded text-xs font-bold text-black ${step === 3 ? 'bg-green-500' : 'bg-yellow-500 hover:bg-yellow-400'}`}>{step === 3 ? "COMPLETE" : "NEXT STEP →"}</button>
// // //           </div>
// // //       </div>
// // //     </div>
// // //   );
...
// //   useFrame((state, delta) => { if (groupRef.current) groupRef.current.rotation.y += delta * (step < 3 ? 12 : 0.8); });
// //   return (
// //     // FIX: Moved position down slightly to center it, Scaled Up to 1.3
...
// //       {/* UI View - Bottom 35% */}
// //       <div className="h-[35%] w-full bg-stone-900 border-t border-stone-800 flex flex-col items-center justify-center p-6 text-center shadow-[0_-10px_20px_rgba(0,0,0,0.5)] relative z-10">
// //           {/* Steps Indicator */}
...
// //           <h3 className="text-white font-bold text-2xl mb-1 font-serif tracking-wide">{steps[step].label}</h3>
// //           <p className="text-gray-400 text-sm mb-6 max-w-md">{steps[step].desc}</p>
          
// //           <div className="flex gap-4 w-full max-w-sm">
// //              <button 
// //                onClick={() => setStep(0)} 
// //                className="px-6 py-3 rounded-xl bg-stone-800 text-white font-bold text-sm hover:bg-stone-700 border border-stone-600 transition-colors"
// //              >
// //                Restart
// //              </button>
// //              <button 
...
// //                disabled={step === 3} 
// //                className={`flex-1 py-3 rounded-xl text-sm font-bold text-black shadow-lg transition-transform hover:scale-105 active:scale-95 ${step === 3 ? 'bg-green-500 cursor-default' : 'bg-yellow-500 hover:bg-yellow-400'}`}
// //              >
...
// // --- KEEP HELPERS (Eye, Moustache, etc) SAME AS BEFORE ---
// const Eye = ({ position, scale = 1 }) => (<group position={position} scale={scale}><mesh scale={[1.2, 0.8, 0.5]}><sphereGeometry args={[0.08, 32, 32]} /><meshStandardMaterial color="white" /></mesh><mesh position={[0, 0, 0.035]} scale={[0.5, 0.5, 0.2]}><sphereGeometry args={[0.08, 32, 32]} /><meshStandardMaterial color="black" /></mesh></group>);
// const Moustache = ({ position }) => (<group position={position}><mesh position={[-0.12, -0.02, 0]} rotation={[0, 0, 0.4]}><capsuleGeometry args={[0.03, 0.25, 4, 16]} /><meshStandardMaterial color="black" /></mesh><mesh position={[0.12, -0.02, 0]} rotation={[0, 0, -0.4]}><capsuleGeometry args={[0.03, 0.25, 4, 16]} /><meshStandardMaterial color="black" /></mesh></group>);
// const Eyebrow = ({ position, rotation }) => (<mesh position={position} rotation={rotation}><torusGeometry args={[0.12, 0.015, 16, 32, 2]} /><meshStandardMaterial color="black" /></mesh>);
// const Smile = ({ position }) => (<mesh position={position} rotation={[0, 0, 3.14]}><torusGeometry args={[0.15, 0.015, 16, 32, 2.2]} /><meshStandardMaterial color="#8B0000" /></mesh>);
...
//         </Canvas>
//       </div>
//       <div className="h-[35%] w-full bg-stone-900 border-t border-stone-800 p-6 flex flex-col items-center justify-center text-center">
...
//              <button onClick={() => setStep(0)} className="px-6 py-3 rounded-xl bg-stone-700 text-white font-bold border border-stone-500 hover:bg-stone-600">Restart</button>
//              <button onClick={() => step < 3 && setStep(step + 1)} disabled={step === 3} className={`flex-1 py-3 rounded-xl font-bold text-black shadow-lg ${step === 3 ? 'bg-green-500' : 'bg-yellow-500 hover:bg-yellow-400'}`}>{step === 3 ? "Completed" : "Next Step →"}</button>
//           </div>
//       </div>
//     </div>
//   );
  ...
  return (
    <div className="flex flex-col h-full w-full bg-stone-950">
      ...
      <div className="h-[30%] w-full bg-stone-900 border-t border-stone-700 flex flex-col items-center justify-center p-4 text-center relative z-10">
          ...
          <h3 className="text-yellow-500 font-bold text-2xl mb-1 tracking-wide">{steps[step].label}</h3>
          <p className="text-gray-300 text-sm mb-4 max-w-lg leading-relaxed">{steps[step].desc}</p>
          ...
          <div className="flex gap-4 w-full max-w-md">
             ...
             <button 
               onClick={() => step < 3 && setStep(step + 1)} 
               disabled={step === 3} 
               className={`flex-1 py-3 rounded-xl text-sm font-bold text-black shadow-[0_0_15px_rgba(234,179,8,0.3)] transition-all hover:scale-105 active:scale-95 
